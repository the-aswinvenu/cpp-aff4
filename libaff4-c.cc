...
public:
    void log(const spdlog::details::log_msg& msg) {
        ...
        // Extract the message payload (formatted string view)
        std::string formatted(msg.payload.begin(), msg.payload.end());
        
        char* str = new char[formatted.size() + 1];
        std::strncpy(str, formatted.c_str(), formatted.size());
        str[formatted.size()] = '\0';

        AFF4_Message* m = new AFF4_Message{static_cast<unsigned int>(msg.level), str, nullptr};
...
static std::shared_ptr<spdlog::logger> setup_c_api_logger() {
    ...
    sinks.push_back(std::make_shared<LogSink>());
    auto logger = std::make_shared<spdlog::logger>(aff4::LOGGER, sinks.begin(), sinks.end());
