        ...
        try {
            auto file_sink = std::make_shared<spdlog::sinks::basic_file_sink_mt>(
                GetArg<TCLAP::ValueArg<std::string>>(
                    "logfile")->getValue(), true);
            
            std::vector<spdlog::sink_ptr> sinks;
            sinks.push_back(file_sink);
            
            // Keep existing console sink if available
            auto existing_sinks = resolver.logger->sinks();
            for (auto& sink : existing_sinks) {
                if (sink) {
            ...
            resolver.logger = std::make_shared<spdlog::logger>(
                aff4::LOGGER, sinks.begin(), sinks.end());
            resolver.logger->set_pattern("%Y-%m-%d %T %L %v");
        } catch (const spdlog::spdlog_ex& ex) {
            resolver.logger->error("Failed to set up file logging: {}", ex.what());
        }
    }
    ...
    for (std::string glob : inputs) {
        for (std::string input : GlobFilename(glob)) {
            ...
            RETURN_IF_ERROR(NewFileBackedObject(
                                &resolver, input, "read",
                                input_stream));
            resolver.logger->info("Adding {} as {}", input, input_stream->urn.SerializeToString());
