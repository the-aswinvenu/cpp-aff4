...
AFF4Status ZipFile::CreateMemberStream(
    ...
    resolver->logger->debug("Creating ZipFileSegment {}", result->urn.SerializeToString());
...
AFF4Status ZipFileSegment::Flush() {
    if (IsDirty()) {
        ...
        auto backing_store = owner->backing_stream.get();
        resolver->logger->debug("Writing member {}", urn.SerializeToString());
        std::unique_ptr<ZipInfo> zip_info(new ZipInfo());
        ...
        resolver->logger->debug("{} is dirtied by segment {}",
                                owner->urn.SerializeToString(), urn.SerializeToString());
...
AFF4Status ZipFile::StreamAddMember(URN member_urn, AFF4Stream& stream,
    ...
    resolver->logger->debug("Writing member {} at {:x}", member_urn.SerializeToString(),
                            backing_stream->Tell());
