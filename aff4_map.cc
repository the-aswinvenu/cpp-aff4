...
AFF4Status AFF4Map::OpenAFF4Map(
    ...
    while (std::getline(f, line)) {
        ...
        resolver->logger->debug("MAP: Opened {} {} for target {}",
                                target->urn.SerializeToString(), line,  map_obj->targets.size());
    ...
    *length = 0;
    ...
    while (remaining > 0) {
        ...
        resolver->logger->debug("MAP: Reading {} @ {}",
                                target_stream->urn.SerializeToString(), offset_in_target);
...
void AFF4Map::Dump() {
    for (auto it : map) {
        uint64_t map_offset = it.second.map_offset;
        uint64_t target_offset = it.second.target_offset;
        uint64_t length = it.second.length;
        uint32_t target_id = it.second.target_id;
        resolver->logger->info("Key: {}  map_offset={:x} target_offset={:x} length={:x} target_id={} ",
                               it.first, map_offset, target_offset,
                               length, target_id);
    }
}
...
AFF4Status AFF4Map::CopyStreamFromMap(
    ...
    source->resolver->logger->debug("Copy Map Stream {} -> {} ", source->urn.SerializeToString(), dest->urn.SerializeToString());
